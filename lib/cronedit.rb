...
# ** some major changes to handling of crontab files
# *** removed automatic ID generation - previously generated automatic IDs (1)
#     can conflict with automatic IDs generated by future executions
# *** IDs are no longer allowed to conflict within a file.  This (it even had
#     a test case documenting this) allowed cron entries to be erroneously
#     removed.
# *** IDs no longer require a job entry after (comment, variable declaration)
...
module CronEdit
    self::VERSION = '1.0.0' unless defined? self::VERSION
    ...
    protected
        ...
        def parseCrontabFull anIO
                ...
                anIO.each_line { |l|
                    ...
                    elsif id      # line after ID may be a cron entry, but can be anything. Regardless, it will be managed by this system.
                        lines << id #add placeholder to lines
                        key = id.id
                        raise "#{key} is duplicated" if entries[key.to_s]
                        entries[key.to_s]=l
                        id = nil
                    else          # add this line as is (variable definition, blank line, unmanaged cron entry, etc.)
                        lines << l
                        id = nil
                    end
                }
